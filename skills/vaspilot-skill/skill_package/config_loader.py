...
def get_potcar_skill_path() -> Optional[Path]:
    """Auto-detect vasp-potcar-skill path (script: .claude/commands/potcar_skill.py)"""
    # Check common locations: 同目录下的 vasp-potcar-skill，或嵌套的 vasp-potcar-skill/vasp-potcar-skill
    base_candidates = [
    ...
    if os.environ.get("VASP_POTCAR_SKILL_PATH"):
        base_candidates.insert(0, Path(os.environ["VASP_POTCAR_SKILL_PATH"]))

    for path in base_candidates:
        # 直接在该目录下 .claude/commands/potcar_skill.py
        skill_script = path / ".claude" / "commands" / "potcar_skill.py"
        if skill_script.exists():
            return skill_script
        # 嵌套结构: vasp-potcar-skill/vasp-potcar-skill/.claude/commands/potcar_skill.py
        skill_script = path / "vasp-potcar-skill" / ".claude" / "commands" / "potcar_skill.py"
        if skill_script.exists():
            return skill_script
