...
class POTCARGenerator:
    ...
    def get_recommendation(
        ...
        try:
            # 在 skill 根目录执行，便于 potcar_skill 导入 src 下模块
            cwd = self.skill_path.parent.parent.parent if self.skill_path else None
            result = subprocess.run(
                cmd,
                capture_output=True,
                text=True,
                timeout=60,
                cwd=str(cwd) if cwd and cwd.exists() else None,
            )

            if result.returncode == 0:
                output = result.stdout.strip()
                # 先尝试整体解析（vasp-potcar-skill 输出为 indent 多行 JSON）
                try:
...
# POTCAR_DIR on HPC must match config hpc.potcar_dir (e.g. {potcar_dir})
...
"""
    ...
    def generate_for_structure(
        ...
        # vasp-potcar-skill 返回 potcar_types，fallback/其他可能返回 recommendations
        potcar_types = rec_result.get("recommendations") or rec_result.get("potcar_types") or {}
