...
**AI Research Agent** is a community-driven Deep Research framework that builds upon the incredible work of the open source community. Our goal is to combine language models with specialized tools for tasks like web search, crawling, and Python code execution, while giving back to the community that made this possible.

Currently, AI Research Agent has officially entered the [FaaS Application Center of Volcengine](https://console.volcengine.com/vefaas/region:vefaas+cn-beijing/market). Users can experience it online through the [experience link](https://console.volcengine.com/vefaas/region:vefaas+cn-beijing/market/deerflow/?channel=github&source=deerflow) to intuitively feel its powerful functions and convenient operations. At the same time, to meet the deployment needs of different users, AI Research Agent supports one-click deployment based on Volcengine. Click the [deployment link](https://console.volcengine.com/vefaas/region:vefaas+cn-beijing/application/create?templateId=683adf9e372daa0008aaed5c&channel=github&source=deerflow) to quickly complete the deployment process and start an efficient research journey.
...
In this demo, we showcase how to use AI Research Agent to:
...
AI Research Agent is developed in Python, and comes with a web UI written in Node.js. To ensure a smooth setup process, we recommend using the following tools:
...
# Configure conf.yaml for your LLM model, API keys, and all environment variables
# All configuration (including environment variables) is now centralized in conf.yaml
# Please refer to 'docs/configuration_guide.md' for more details
# For local development, you can use Ollama or other local models
cp conf.yaml.example conf.yaml

# Edit conf.yaml and configure:
# - LLM models and API keys (BASIC_MODEL, MODELS sections)
# - Search engine API keys (ENV.TAVILY_API_KEY, ENV.BRAVE_API_KEY, etc.)
# - RAG provider settings (ENV.RAG_PROVIDER, ENV.RAGFLOW_API_URL, etc.)
# - Database connection (ENV.LANGGRAPH_CHECKPOINT_DB_URL) if using PostgreSQL
# - All other environment variables in the ENV section

# Initialize database (if using PostgreSQL for persistence)
...
Open your browser and visit [`http://localhost:3002`](http://localhost:3002) to explore the web UI.
...
AI Research Agent supports multiple search engines that can be configured in your `conf.yaml` file:

- **Tavily** (default): A specialized search API for AI applications
  - Requires `ENV.TAVILY_API_KEY` in your `conf.yaml` file
...
- **Brave Search**: Privacy-focused search engine with advanced features
  - Requires `ENV.BRAVE_API_KEY` in your `conf.yaml` file
...
- **Searx/SearxNG**: Self-hosted metasearch engine
  - Requires `ENV.SEARX_HOST` to be set in the `conf.yaml` file
  - Supports connecting to either Searx or SearxNG

To configure your preferred search engine, set the `ENV.SEARCH_API` variable in your `conf.yaml` file:

```yaml
ENV:
  SEARCH_API: "tavily"  # Choose one: tavily, duckduckgo, brave_search, arxiv
  TAVILY_API_KEY: "your-api-key"  # If using Tavily
```

### Private Knowledgebase

AI Research Agent support private knowledgebase such as ragflow and vikingdb, so that you can use your private documents to answer questions.

- **[RAGFlow](https://ragflow.io/docs/dev/)**：open source RAG engine
   ```yaml
   # Configure in conf.yaml ENV section
   ENV:
...
AI Research Agent implements a modular multi-agent system architecture designed for automated research and code analysis. The system is built on LangGraph, enabling a flexible state-based workflow where components communicate through a well-defined message passing system.
...
AI Research Agent now includes a Text-to-Speech (TTS) feature that allows you to convert research reports to speech. This feature uses the volcengine TTS API to generate high-quality audio from text. Features like speed, volume, and pitch are also customizable.
...
AI Research Agent uses LangGraph for its workflow architecture. You can use LangGraph Studio to debug and visualize the workflow in real-time.

#### Running LangGraph Studio Locally

AI Research Agent includes a `langgraph.json` configuration file that defines the graph structure and dependencies for the LangGraph Studio. This file points to the workflow graphs defined in the project and automatically loads environment variables from the `.env` file.
...
AI Research Agent supports LangSmith tracing to help you debug and monitor your workflows. To enable LangSmith tracing:
...
```bash
docker build -t ai-research-agent-api .
```

Final, start up a docker container running the web server:

```bash
# Replace ai-research-agent-api-app with your preferred container name
# Start the server then bind to localhost:8000
docker run -d -t -p 127.0.0.1:8000:8000 --env-file .env --name ai-research-agent-api-app ai-research-agent-api

# stop the server
docker stop ai-research-agent-api-app
```

### Docker Compose (include both backend and frontend)

AI Research Agent provides a docker-compose setup to easily run both the backend and frontend together:
...
> [!WARNING]
> If you want to deploy the AI Research Agent into production environments, please add authentication to the website and evaluate your security check of the MCPServer and Python Repl. 

## Examples

The following examples demonstrate the capabilities of AI Research Agent:
...
AI Research Agent includes a human in the loop mechanism that allows you to review, edit, and approve research plans before they are executed:
...
AI Research Agent is built upon the incredible work of the open-source community. We are deeply grateful to all the projects and contributors whose efforts have made AI Research Agent possible. Truly, we stand on the shoulders of giants.

We would like to extend our sincere appreciation to the following projects for their invaluable contributions:

- **[LangChain](https://github.com/langchain-ai/langchain)**: Their exceptional framework powers our LLM interactions and chains, enabling seamless integration and functionality.
- **[LangGraph](https://github.com/langchain-ai/langgraph)**: Their innovative approach to multi-agent orchestration has been instrumental in enabling AI Research Agent's sophisticated workflows.
...
A heartfelt thank you goes out to the core authors of `AI Research Agent`, whose vision, passion, and dedication have brought this project to life:

- **[Daniel Walnut](https://github.com/hetaoBackend/)**
- **[Henry Li](https://github.com/magiccube/)**

Your unwavering commitment and expertise have been the driving force behind AI Research Agent's success. We are honored to have you at the helm of this journey.
