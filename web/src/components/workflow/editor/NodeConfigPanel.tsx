...
import { X, Plus, Trash2, CheckCircle2, XCircle, Loader2, Clock } from "lucide-react";
  ...
  if (!result && status === "pending") {
    return (
      <div className="flex flex-col items-center justify-center h-full text-muted-foreground p-8">
        <p>暂无运行结果</p>
        <p className="text-xs mt-2">点击"试运行"查看结果</p>
      </div>
    );
  }
      ...
      {result?.inputs && (
        <div className="space-y-2">
          ...
          <div className="bg-muted rounded-md p-2 overflow-auto max-h-[200px]">
            <pre className="text-xs font-mono">{JSON.stringify(result.inputs, null, 2)}</pre>
          </div>
        </div>
      )}
      ...
      {result?.outputs && (
        <div className="space-y-2">
          ...
          <div className="bg-muted rounded-md p-2 overflow-auto max-h-[300px]">
             <pre className="text-xs font-mono">{JSON.stringify(result.outputs, null, 2)}</pre>
          </div>
        </div>
      )}
    </div>
...
function LoopNodeConfig({ node, nodeData, nodes = [], edges = [], onUpdate }: NodeConfigProps) {
  const [loopCount, setLoopCount] = useState(
    nodeData.loopCount?.toString() || nodeData.loop_count?.toString() || ""
  );
  ...
  const [loopVariables, setLoopVariables] = useState(
    nodeData.loopVariables || nodeData.loop_variables || []
  );
  ...
  const addLoopVariable = () => {
    ...
    setLoopVariables([...loopVariables, newVariable]);
    onUpdate(node.id, {
      ...nodeData,
      loopVariables: [...loopVariables, newVariable],
    });
  };

  const removeLoopVariable = (index: number) => {
    const newVariables = loopVariables.filter((_, i) => i !== index);
    setLoopVariables(newVariables);
    onUpdate(node.id, { ...nodeData, loopVariables: newVariables });
  };

  const updateLoopVariable = (index: number, field: string, value: any) => {
    const newVariables = [...loopVariables];
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        {loopVariables.length === 0 ? (
          ...
          <div className="space-y-2">
            {loopVariables.map((variable: any, index: number) => (
              <div key={index} className="border rounded-md p-3 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    onClick={() => removeLoopVariable(index)}
                  >
                    <Trash2 className="h-4 w-4" />
                  </Button>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <Label className="text-xs">变量标签</Label>
                    <Input
                      value={variable.label || ""}
                      onChange={(e) =>
                        updateLoopVariable(index, "label", e.target.value)
                      }
                      placeholder="变量名"
                      className="h-8"
                    />
                  ...
                  <div>
                    ...
                    <Select
                      value={variable.varType || "string"}
                      onValueChange={(value) =>
                        updateLoopVariable(index, "varType", value)
                      }
                    >
                      <SelectTrigger className="h-8">
                        <SelectValue />
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="array_string">字符串数组</SelectItem>
                        <SelectItem value="array_number">数字数组</SelectItem>
                        <SelectItem value="array_object">对象数组</SelectItem>
                        <SelectItem value="array_boolean">布尔值数组</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                </div>
                <div>
                  ...
                  <Select
                    value={variable.valueType || "constant"}
                    onValueChange={(value) =>
                      updateLoopVariable(index, "valueType", value)
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="variable">变量（从上游节点获取）</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                {variable.valueType === "constant" && (
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateLoopVariable(index, "value", e.target.value)
                      }
                      placeholder="输入变量值"
                      className="h-8"
                    />
