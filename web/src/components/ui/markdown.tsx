...
export function Markdown({
  ...
  const components: ReactMarkdownOptions["components"] = useMemo(() => {
    return {
      ...
      img: ({ src, alt }) => {
        const isBase64 = (src as string)?.startsWith('data:image');
        if (isBase64) {
          // For base64 images, render directly without link
          return <Image className="rounded" src={src as string} alt={alt ?? ""} />;
        } else {
          // For static files and external URLs, render directly
          // Remove link wrapper to display images inline
          return <Image className="rounded" src={src as string} alt={alt ?? ""} />;
        }
      },
    };
