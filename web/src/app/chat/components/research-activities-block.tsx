  ...
  if (activityIds.length === 0) {
    return (
      <div className={cn("flex flex-col items-center justify-center py-12", className)}>
        <p className="text-gray-500 dark:text-gray-400">暂无执行活动</p>
      </div>
    );
  }
...
const ActivityMessage = React.memo(({ messageId }: { messageId: string }) => {
  ...
  // Clean content: remove HTML comments and fix incomplete Markdown
  // Move useMemo to top level (before any conditions) to follow Rules of Hooks
  const cleanedContent = useMemo(() => {
    if (!message?.content) return "";
    let content = message.content;
    // Remove HTML comments (especially <!-- MOLECULAR_IMAGE_ID:... -->)
    content = content.replace(/<!--[\s\S]*?-->/g, '');
    // Remove standalone incomplete Markdown markers (like ** or * without pairs)
    // This regex matches ** or * at the start or end of lines with only whitespace
  ...
  if (message?.agent) {
    // Activity page should only show researcher and coder messages, not reporter or planner
    if (message.agent !== "reporter" && message.agent !== "common_reporter" && message.agent !== "planner" && message.agent !== "molecular_planner") {
      // If message has toolCalls, show it even if content is empty
      if (message.toolCalls && message.toolCalls.length > 0) {
        // Tool calls are handled by ActivityListItem, so just return null here
        return null;
      }
      
      // If content exists, clean and display it
      // Only render if content is not empty after cleaning (but allow very short messages)
    ...
    if (!message.isStreaming && message.toolCalls?.length) {
      for (const toolCall of message.toolCalls) {
        ...
        if (toolCall.name === "web_search") {
          ...
          return <SearchToolCall key={toolCall.id} toolCall={toolCall} />;
        } else if (toolCall.name === "visit") {
          return <VisitToolCall key={toolCall.id} toolCall={toolCall} />;
        } else {
...
// DeepResearch工具调用组件
function SearchToolCall({ toolCall }: { toolCall: ToolCallRuntime }) {
  const t = useTranslations("chat.research");
  const searching = useMemo(() => {
    return toolCall.result === undefined;
  }, [toolCall.result]);
      ...
      {searchResults.length > 0 && (
        <div className="mt-3 space-y-2">
          {searchResults.map((result: any, index: number) => (
            ...
            >
              <div className="font-medium text-sm">{result.title || "Search Result"}</div>
              {result.url && (
                <a 
                  href={result.url} 
                  target="_blank" 
...
function VisitToolCall({ toolCall }: { toolCall: ToolCallRuntime }) {
  const t = useTranslations("chat.research");
  const visiting = useMemo(() => {
    return toolCall.result === undefined;
  }, [toolCall.result]);
  
  const visitArgs = useMemo(() => {
    try {
      return toolCall.args as { url?: string[]; goal?: string };
    } catch {
      return { url: [], goal: "" };
    }
  }, [toolCall.args]);
  ...
  return (
    <section className="mt-4 pl-4">
      ...
      {visitArgs.url && visitArgs.url.length > 0 && (
        <ul className="mt-2 flex flex-wrap gap-4">
          {visitArgs.url.map((url, index) => (
            <motion.li
              key={index}
              className="text-muted-foreground bg-accent flex h-40 w-40 gap-2 rounded-md px-2 py-1 text-sm"
              initial={{ opacity: 0, y: 10 }}
              ...
              transition={{ delay: index * 0.1 }}
            >
              <FavIcon className="mt-1" url={url} title="" />
              <a
                className="h-full flex-grow overflow-hidden text-ellipsis whitespace-nowrap"
                href={url}
                target="_blank"
      ...
      {extractedContent && (
        <div className="mt-3">
          <Accordion type="single" collapsible className="w-full">
            <AccordionItem value="extracted-content">
              <AccordionTrigger>
                <div className="flex items-center font-medium italic">
              ...
              <AccordionContent>
                <div className="bg-accent max-h-[400px] max-w-[560px] overflow-y-auto rounded-md p-3 text-sm">
