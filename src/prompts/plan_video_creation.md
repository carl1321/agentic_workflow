...
- **第三阶段：动画片段生成** — 必须规划为 .mp4 产出，执行层会强制调用 **video_generation_tool** 生成视频；不得规划为「生成 Python 脚本」或 .py/.md 等替代产出。
...
- **output_relpath 后缀必须与产物类型一致**：文生图/图片任务必须用 .png 或 .jpg（如 role_scene.png），**动画/视频片段必须用 .mp4**（执行层会强制调用 video_generation_tool，不得规划为脚本或 .md 等），文本/分镜用 .md、.json。禁止将图片任务产出写成 .json。
